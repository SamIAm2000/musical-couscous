...
//   document.getElementById('pew').addEventListener('click', handleButtonClick);
...
function handleAnimationEnd(event) {
  ...
  // Function to handle button click
  function handleButtonClick() {
    // Create a new instance of the pot element
    var pot = document.createElement('div');
    pot.classList.add('pot');
    document.querySelector('.lasercontainer').appendChild(pot);

    // Add the animation class to start the animation
    pot.classList.add('animate');

    // Add event listener to detect animation end for the pot
    pot.addEventListener('animationend', handleAnimationEnd, { once: true });
  }
...
function createBlasterSound(audioCtx, duration) {
  ...
  for (let i = 0; i < bufferSize; i++) {
    if (i < attackDuration * audioCtx.sampleRate) {
      ...
      const t =
        (i - attackDuration * audioCtx.sampleRate) /
        (decayDuration * audioCtx.sampleRate);
...
function createBlasterSoundWithEcho(audioCtx, duration) {
  ...
  osc3.frequency.exponentialRampToValueAtTime(
    0.00001,
    audioCtx.currentTime + duration - 3
  ); // Ramp down to 0 Hz
  ...
  osc4.frequency.exponentialRampToValueAtTime(
    0.00001,
    audioCtx.currentTime + duration - 3.5
  ); // Ramp down to 0 Hz
...
function startAnimation() {
  ...
  var startButton = document.getElementById("pew");
  var pot = document.getElementById("pot");

  //   // Get the position of the start button
  //   var startButtonRect = startButton.getBoundingClientRect();

  //   pot.style.top = startButtonRect.top + 'px';
  //   pot.style.left = startButtonRect.left + 'px';

  //   // Set the initial position of the pot to match the start button
  //   //pot.style.left = startButtonRect.left + 'px';
  //     pot.style.animationPlayState = 'running';
  pot.classList.toggle("animate");

  //pot.classList.add("animate");

  //   setTimeout(function() {
  //     pot.classList.remove("animate");
  //   }, 1000); // 500 is the same time as the CSS animation

  pot.classList.remove("classname"); // reset animation
  void pot.offsetWidth; // trigger reflow
  pot.classList.add("classname"); // start animation
}

// function handleAnimationEnd() {
//   document.getElementById("pot").classList.remove("animate"); // Remove animate class
// }

// // Function to handle button click
// function handleButtonClick() {
//   var pot = document.getElementById("pot");
//   pot.classList.add("animate"); // Add animate class to start animation
